...
| 阶段 | 里程碑 | 核心产出 |
|------|--------|----------|
| **M1** | 数据聚合与全局索引 | 多源数据按时间戳对齐为 GroupData，构建 `Subject_Data_Global_Reference.xlsx` 全局受试者索引 |
| **M2** | 量表计分与数据预处理 | GSE / ITS / SSQ / PANAS 标准化计分，按决策点（DP1/DP2/DP3）切分时间窗口数据 |
| **M3** | 运动学与眼动指标提取 | 速度、加速度、移动距离、疏散耗时、注视点分类、Lyapunov 指数、注视路径距离/速度、跟随比例 |
| **M4** | 统计检验 | 正态性检验（Shapiro-Wilk、K-S）、参数/非参数组间比较（t 检验、Mann-Whitney、ANOVA、Kruskal-Wallis、Friedman） |
| **M5** | 贝叶斯层次 Logistic 回归 | PyMC 建模（V1→V8），含固定效应、随机效应、ROC/PR 校准曲线评估，按三决策点独立建模 |
| **M6** | 混合效应 Logit 模型 | pymer4 / R-lme4 GLMM（V1→V5），频率学派视角交叉验证，按三决策点独立建模 |
| **M7** | 眼动模式聚类分析 | DBSCAN 2D/3D 聚类 × 4 场景 × 4 注视目标，KDE 热力图、拓扑图、极坐标可视化 |
| **M8** | 组间/组内比较与可视化 | 箱型图/小提琴图、Spearman 相关性热力图、DTW 轨迹相似度、决策点跟随比例比较 |
| **M9** | 图表终稿与论文配图 | 问卷数据分布图、极坐标注视热力图、轨迹对比图、饼图、后验森林图——全部 ≥300 DPI 出版级输出 |
