...
实验数据采集于长春与深圳两地，受试者按照领导者类型分为 Passenger（乘客）、Robot（机器人）、Security（安保）三组，覆盖 A1 至 B5 共 7 个实验关卡。
...
- **功能描述**：通过扫描长春与深圳两地的受试者数据目录，建立受试者昵称到数据文件夹路径的映射，并导出为全局索引表。
- **核心逻辑**：
  - 从基本情况问卷中读取所有受试者的飞书昵称与真实姓名。
  - 遍历 `受试者采集数据_长春` 和 `受试者采集数据_深圳` 目录树，利用昵称模糊匹配受试者文件夹。
