...
scale_data_max_index = 22

def find_file():
    global global_current_script_path
    global_current_script_path = os.path.abspath(__file__)
    global global_file_path
    global_file_path = global_current_script_path.replace("Scripts\After_PANAS_Calculation.py", "")
    global_file_path = global_file_path + "问卷数据" + os.sep + "积极消极情绪问卷_实验后" + os.sep
...
def output():
    ...
    global_output_path = global_current_script_path.replace("Scripts\After_PANAS_Calculation.py", "")
    now = datetime.datetime.now()
    timestamp = now.strftime("%Y-%m%d-%H-%M")
    global_output_path = global_output_path + "Results" + os.sep + "After-PANAS-Results-" + timestamp + ".xlsx"
