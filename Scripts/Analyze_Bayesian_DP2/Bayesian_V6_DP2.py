...
if __name__ == "__main__":
    ...
    for var1, var2 in all_pairs:
        ...
        with pm.Model(encode_cpp=True) as interaction_model:
        ...
        with open(filename, 'w') as f:
            ...
            with pd.option_context('display.max_columns', None,
                                   ...
                                   'display.width', 1000):
                interaction_summary = pm.summary(trace, var_names=[f'beta_{interaction_term}'], hdi_prob=0.89)
                f.write("Interaction Coefficient Summary:\n")
                f.write(interaction_summary.to_string())
                f.write("\n\n")

                f.write("Main Effect Coefficients Summary:\n")
                main_effect_summary = pm.summary(trace, var_names=[f'beta_{var1}', f'beta_{var2}'], hdi_prob=0.89)
                f.write(main_effect_summary.to_string())
                f.write("\n\n")

                f.write("Full Model Summary:\n")
                f.write(pm.summary(trace, hdi_prob=0.89).to_string())
                f.write(f"\n\nTime elapsed: {elapsed_time:.2f} seconds")

        plt.figure()
        az.plot_posterior(trace, var_names=[f'beta_{interaction_term}', f'beta_{var1}', f'beta_{var2}'], hdi_prob=0.89)
