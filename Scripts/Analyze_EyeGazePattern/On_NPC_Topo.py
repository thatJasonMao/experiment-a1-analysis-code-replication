...
def calculate_heatmap_center_and_percentage_boundary(heatmap, xedges, yedges, top_percentage):
    ...
    total_weighted_x = np.sum(heatmap * x_bin_centers[:, None])
    total_weighted_y = np.sum(heatmap * y_bin_centers[None, :])
    center_x = total_weighted_x / total_intensity
    center_y = total_weighted_y / total_intensity
...
def plot_topology(data, title, ax, vmin, vmax):
    ...
    center, boundary_points, sorted_intensity, total_intensity = calculate_heatmap_center_and_percentage_boundary(heatmap, xedges, yedges, global_top_percentage)
    ...
    ax.text(
        -6.3, 19, f"Mass Center: ({center[0]:.3f}, {center[1]:.3f})",
        fontsize=10, color='black', bbox=dict(boxstyle="round,pad=0.2", facecolor="white", edgecolor="black")
    )
    ax.text(
        -8.4, 17.5, f"Top 10% Concentration Index: {concentration_index:.3f}",
        fontsize=10, color='black', bbox=dict(boxstyle="round,pad=0.2", facecolor="white", edgecolor="black")
    )
    ax.text(
        -5.2, 16, f"Elongation Index: {elongation_index:.3f}",
        fontsize=10, color='black', bbox=dict(boxstyle="round,pad=0.2", facecolor="white", edgecolor="black")
    )
    ...
    ax.set_ylabel('Y Position', rotation=270, va='bottom', ha='center', labelpad=20)
