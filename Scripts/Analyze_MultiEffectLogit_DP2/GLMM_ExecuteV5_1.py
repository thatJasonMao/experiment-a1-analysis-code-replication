...
plt.errorbar(fixed_effects['Estimate'], fixed_effects['Variable'],
             ...
             fmt='o', color='black', ecolor='darkorange', capsize=5,
             elinewidth=2.5, mew=2.5, markersize=5)

plt.axvline(0, color='r', linestyle='--', linewidth=2.5)
...
plt.rcParams['font.family'] = 'Times New Roman'
plt.plot(fpr, tpr, color='darkorange', lw=3.5,
         label=f'ROC Curve (AUC = {roc_auc:.3f})')
plt.plot([0, 1], [0, 1], color='navy', lw=3.5, linestyle='--')
plt.xlabel('False Positive Rate', fontsize=28)

plt.ylabel('True Positive Rate', fontsize=28)

plt.title('Receiver Operating Characteristic', fontsize=28)

plt.legend(loc="lower right", fontsize=28)

plt.tick_params(axis='both', labelsize=28)

plt.savefig(f'Results/roc_curve_{current_version}.png', dpi=600, bbox_inches='tight')
plt.close()
