...
def get_dp3_result_data(subject_name, level_name):
                ...
                if raw_result == "Null" or raw_result == "Error":
                    ...
                    if "A" in raw_result:
                        if "R" in raw_result:
                            ...
                            dp_result = "No"
    if dp_result == "Null":
        print(f"Name:{subject_name} Level:{level_name} DP3 Result Is Null.")
    return dp_result
    ...
    with open(out_put_file, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        for key in subject_gse_dict.keys():
            ...
            while counter < len(target_levels):
                ...
                dp_result = get_dp3_result_data(key, level)
    ...
    with (open(out_put_file, 'r', newline='', encoding='utf-8',errors="ignore") as infile,
        ...
        for row in reader:
            if "Null" not in str(row) and "Error" not in str(row):
                counter = counter + 1
                writer.writerow(row)
        print(f"Total DP3 decision data records collected / 共统计到DP3决策数据{counter - 1}组")
