...
def extract_speed_by_group():
    global avg_acc_infos
    target_file_path = get_parent_folder_path() + os.sep + "GroupData_Avg_Acc_XZ" + os.sep + "Result.csv"
    with open(target_file_path, 'r', newline='', encoding='utf-8') as csvfile:
        reader = csv.reader(csvfile)
        next(reader)
        for row in reader:
            if len(row) > 1:
                avg_acc_infos.append(row)
    print("Total Level Speed Count:" + str(len(avg_acc_infos)))
    output_group_security(get_parent_folder_path() + os.sep + "GroupData_Avg_Acc_XZ")
    output_group_passenger(get_parent_folder_path() + os.sep + "GroupData_Avg_Acc_XZ")
    output_group_robot(get_parent_folder_path() + os.sep + "GroupData_Avg_Acc_XZ")
