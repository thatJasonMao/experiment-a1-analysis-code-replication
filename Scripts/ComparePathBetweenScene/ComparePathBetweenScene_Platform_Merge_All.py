...
def draw_level(subject_traj, leader_traj, name):
    ...
    show_leader = False
    if show_leader and any(target_level != "A1" for target_level in total_level_list):
        for traj in negated_scaled_leader_traj:
            x_coords = [point[0] for point in traj]
            y_coords = [point[1] - y_shift for point in traj]
            plt.plot(x_coords, y_coords, marker='o', color='tomato', markersize=0.06, linewidth=0.02)
    ...
    plt.savefig("All_Levels_Platform_" + name + '.png', dpi=300, bbox_inches='tight', transparent=True)
    plt.close()
