...
def build_traj_list():
    ...
    print(f"关卡 {target_level}  Subject轨迹+速度 数据数量数：" + str(len(subject_info_total)))
...
def draw_level(subject_traj, name):
    ...
    for traj in negated_scaled_subject_traj:
        new_traj = [(x, y - y_shift, z) for x, y, z in traj]
        offset_subject_traj.append(new_traj)

    new_flat_traj = [item for sublist in offset_subject_traj for item in sublist]
    ...
    full_min_x = min(min_x, x_offset)
    full_max_x = max(max_x, x_offset + base_width)
    full_min_y = min(min_y, y_offset)
    full_max_y = max(max_y, y_offset + base_height)

    x_range = full_max_x - full_min_x
    y_range = full_max_y - full_min_y
    ...
    im = plt.imshow(statistic.T, origin='lower', cmap=custom_cmap, extent=[x_edge[0], x_edge[-1], y_edge[0], y_edge[-1]], alpha=1)
    ...
    cbar = plt.colorbar(im,  shrink=1, label="Speed(m/s)", pad=-0.4)

    cbar.set_label("Speed(m/s)", fontsize=12)
    cbar.ax.tick_params(labelsize=12)

    plt.title(f'Velocity Distribution Scenario:{target_level} Group:{name}', x=0.6, fontsize=15)

    # plt.subplots_adjust(bottom=0)

    plt.xlim(full_min_x, full_max_x)
    plt.ylim(full_min_y, full_max_y)

    pic_name = f'{target_level}_Plane_Velocity_Hall_{name}.png'
    plt.savefig(pic_name, dpi=300, bbox_inches='tight')

    image = Image.open(f'{target_level}_Plane_Velocity_Hall_{name}.png')
    width, height = image.size
    cropped_image = image.crop((200, 0, width, height))
    cropped_image.save(f'{target_level}_Plane_Velocity_Hall_{name}.png')
