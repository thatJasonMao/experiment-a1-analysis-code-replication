...
target_level_tail = '_B1'

data = pd.read_csv('TrajDTW.csv')
...
def execute():
    for _, row in a1_data.iterrows():
        ...
        subject_dict[subject_name] = row['DTW']

    print("Subject DTW value dict: / 受试者DTW值字典:")
    for subject, mle_value in subject_dict.items():
        print(f"{subject}: {mle_value}")

    out_put_folder = "DTW_By_Level"
    ...
    with open(out_put_folder + os.sep + "Security" + target_level_tail + ".csv", 'w', newline='', encoding='utf-8') as csvfile:
        for key in subject_dict.keys():
            if key in group_security:
                csvfile.write(key + "," + str(subject_dict[key]) + "\n")
