...
def get_gaze_on_leader(path):
        ...
        for row in reader:
            if len(row) > 1:
                if "Leader" in str(row[25]):
                    ...
                    first_hit_point = row[22] + "," + row[23] + "," + row[24]
                    new_line = time_stamp + "," + subject_pos + "," + leader_pos + "," + first_hit_point
    ...
    output_folder = get_parent_folder_path() + os.sep + "GroupData_Gaze_On_Leader_Pos_Info"
    if not os.path.exists(output_folder):
        os.mkdir(output_folder)
    output_path = output_folder + os.sep + str(level_name) + ".csv"
    header = "TimeStamp,SubjectPX,SubjectPY,SubjectPZ,LeaderPX,LeaderPY,LeaderPZ,HitPX,HitPY,HitPZ"
