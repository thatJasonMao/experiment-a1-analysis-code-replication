...
def get_gaze_on_leader(path):
    ...
    with open(path, 'r', newline='', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            if len(row) > 1:
                if "NPC" not in str(row[25]) and "Leader" not in str(row[25]):
                    time_stamp = row[0]
                    subject_pos = row[1] + "," + row[2] + "," + row[3]
                    subject_rot = row[7] + "," + row[8] + "," + row[9]
                    obj_pos = row[22] + "," + row[23] + "," + row[24]
                    new_line = time_stamp + "," + subject_pos + "," + subject_rot + "," + obj_pos
                    gaze_info_lines.append(new_line)

    output_folder = get_parent_folder_path() + os.sep + "GroupData_Gaze_On_ENV_Full_Info"
    if not os.path.exists(output_folder):
        os.mkdir(output_folder)
    output_path = output_folder + os.sep + str(level_name) + ".csv"
    header = "TimeStamp,SubjectPosX,SubjectPosY,SubjectPosZ,SubjectRotX,SubjectRotY,SubjectRotZ,ENV_PX,ENV_PY,ENV_PZ"
...
def analyze():
    ...
    for file in target_files:
        ...
        progress_bar.set_description(f"提取受试者对Env的注视")
