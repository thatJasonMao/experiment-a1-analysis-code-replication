...
def get_gaze_on_leader(path):
        ...
        for row in reader:
            if len(row) > 1:
                if "Leader" in str(row[25]):
                    ...
                    leader_yaw = row[42]
                    new_line = time_stamp + "," + subject_pos + "," + subject_rot + "," + leader_pos + "," + gaze_pos + "," + leader_yaw
    ...
    output_folder = get_parent_folder_path() + os.sep + "GroupData_Gaze_On_Leader_Full_Info_Polar_Advanced"
    if not os.path.exists(output_folder):
        os.mkdir(output_folder)
    output_path = output_folder + os.sep + str(level_name) + ".csv"
    header = "TimeStamp,SubjectPosX,SubjectPosY,SubjectPosZ,SubjectRotX,SubjectRotY,SubjectRotZ,LeaderPX,LeaderPY,LeaderPZ,GazePX,GazePY,GazePZ,LeaderYaw"
