...
def read_and_extract():
    ...
    while subject_data_index < len(global_data_reference.keys()):
        ...
        progress_bar.set_description(f"提取溅落在环境中的眼动点")
    ...
    if len(dir_files) == 0:
        ...
        with open(gaze_file_path, mode='r', encoding='utf-8', errors='ignore') as file:
            ...
            for row in csv_reader:
                if len(row) > 0:
                    if "NPC" not in str(row[10]) and "Leader" not in str(row[10]):
                        x = row[7]
                        y = row[8]
                        z = row[9]
                        name = row[10]
                        ...
                        timestamp = row[0]
                        on_env_gaze_data[timestamp] = hit_info

        current_script_path = os.path.abspath(__file__)
        current_folder = os.path.dirname(current_script_path)
        parent_folder = os.path.dirname(current_folder)
        output_raw_path = parent_folder + os.sep + "Results" + os.sep + "Gaze_On_Env" + os.sep + subject_name
        if not os.path.exists(output_raw_path):
            os.mkdir(output_raw_path)
        output_target_file = (parent_folder + os.sep + "Results" + os.sep + "Gaze_On_Env"
        ...
        with open(output_target_file, mode='w', encoding='utf-8', newline='') as file:
            ...
            for key in on_env_gaze_data.keys():
                line_content = key + "," + on_env_gaze_data[key]
