...
def read_and_extract():
    ...
    while subject_data_index < len(global_data_reference.keys()):
        ...
        progress_bar.set_description(f"提取受试者加速度")
...
def build_list(level, data_raw_path, subject_name):
    ...
    if len(dir_files) == 0:
            ...
            for row in csv_reader:
                if len(row) > 0:
                    try:
                        ...
                        acc_y = row[12]
                        acc_z = row[13]
                        acc = f"{acc_x},{acc_y},{acc_z}"
                        timestamp = row[0]
                        acc_info_dict[timestamp] = acc
        ...
        current_folder = os.path.dirname(current_script_path)
        parent_folder = os.path.dirname(current_folder)
        output_raw_path = parent_folder + os.sep + "Results" + os.sep + "Full_Acc" + os.sep + subject_name
        if not os.path.exists(output_raw_path):
            os.mkdir(output_raw_path)
        output_target_file = (parent_folder + os.sep + "Results" + os.sep + "Full_Acc"
                              + os.sep + subject_name + os.sep + global_level_names[level] + ".csv")

        csv_header = "TimeStamp,Acc_X,Acc_Y,Acc_Z"
        with open(output_target_file, mode='w', encoding='utf-8', newline='') as file:
            file.write(csv_header + "\n")
            for key in acc_info_dict.keys():
                line_content = key + "," + acc_info_dict[key]
