...
def read_and_extract():
    ...
    while subject_data_index < len(global_data_reference.keys()):
        ...
        progress_bar.set_description(f"提取受试者速度")
...
def build_list(level, data_raw_path, subject_name):
    ...
    if len(dir_files) == 0:
            ...
            for row in csv_reader:
                if len(row) > 0:
                    try:
                        ...
                        velo_z = row[6]
                        velo_abs = row[10]
                        velo = f"{velo_x},{velo_y},{velo_z},{velo_abs}"
                        timestamp = row[0]
                        speed_info_dict[timestamp] = velo
        ...
        current_folder = os.path.dirname(current_script_path)
        parent_folder = os.path.dirname(current_folder)
        output_raw_path = parent_folder + os.sep + "Results" + os.sep + "Full_Speed" + os.sep + subject_name
        if not os.path.exists(output_raw_path):
            os.mkdir(output_raw_path)
        output_target_file = (parent_folder + os.sep + "Results" + os.sep + "Full_Speed"
                              + os.sep + subject_name + os.sep + global_level_names[level] + ".csv")

        csv_header = "TimeStamp,Velo_X,Velo_Y,Velo_Z,Velo_Abs"
        with open(output_target_file, mode='w', encoding='utf-8', newline='') as file:
            file.write(csv_header + "\n")
            for key in speed_info_dict.keys():
                line_content = key + "," + speed_info_dict[key]
